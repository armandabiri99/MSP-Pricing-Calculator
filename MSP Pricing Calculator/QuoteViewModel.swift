...
import UIKit            // UIImage used in PDF generation
...
final class QuoteViewModel: ObservableObject {
    ...
    @Published var numCameras        = 0      // new feature

    // NVR selection (index into `nvrOptions`)
    @Published var selectedNvr       = 0      // 0:none, 1:8-port … 4:64-port
    ...
    @Published var includeHuntress     = false   // new
    @Published var includeWebroot      = false   // new

    // MARK: – Quote branding (optional)
    ...
    /// Generates a one-page PDF with optional logo & customer/company info.
